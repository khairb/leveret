...
class ScraperResult:
    ...
    Attributes:
        ...
        generated: ``True`` if a new script was generated by the AI
        ...
        auto_fixed: ``True`` when auto-fix triggered regeneration.
            Implies ``generated=True`` (a fresh script was generated).
    """
...
class Scraper:
    ...
    Args:
        ...
        tolerance: How strictly to validate list items against the
            schema. ``STRICT`` requires 100%, ``BALANCED`` (default)
            requires 80%, ``TOLERANT`` requires 50%.
        ...
        protect_script: When ``True``, prevents ``auto_fix`` and
            ``regenerate()`` from overwriting a script you edited by
        ...
        timeout: Maximum seconds for script execution. Defaults to
            600 (10 minutes). Does not limit AI generation time.
        max_retries: Maximum number of retries if the AI fails to
            generate a valid script. Defaults to 5 (6 total attempts).
        auto_fix: Automatically diagnose and regenerate broken cached
            scripts. ``False`` (default) disables auto-fix. ``True``
            enables balanced mode. Pass an :class:`AutoFixMode` for
            fine-grained control.
        stop_on_captcha: When ``True`` (default), the agent stops
            gracefully if it detects a CAPTCHA instead of looping.

        sandbox: Run AI-generated code in a restricted sandbox that
            blocks filesystem, network, and system access. Enabled by
            default. Disable only if you trust the target site.
        validator_agent: Use an additional AI agent to review the
    ...
    def __init__(
        ...
        *,
        # ── What to extract ──
        schema: "Schema",
        tolerance: Tolerance | str = Tolerance.BALANCED,
        # ── Where to cache ──
        ...
        timeout: int = 600,
        max_retries: int = 5,
        auto_fix: bool | AutoFixMode = False,
        stop_on_captcha: bool = True,
        # ── Safety ──
        sandbox: bool = True,
        validator_agent: bool = False,
    ) -> None:
        # -- url --
    ...
    def _resolve_auto_fix_value(value: bool | str | AutoFixMode) -> AutoFixMode:
        ...
        if mode is None:
            raise ConfigError(
                f"auto_fix must be False, True, or an AutoFixMode value "
                f"(got {value!r}). Examples:\n\n"
                f"  auto_fix=True                        # balanced mode\n"
                f"  auto_fix=AutoFixMode.CONSERVATIVE     # conservative mode\n"
                f"  auto_fix=AutoFixMode.AGGRESSIVE       # aggressive mode\n"
                f"  auto_fix=AutoFixMode.ALWAYS           # regenerate every run"
            )
        return mode
    ...
    async def async_run(
        ...
        *,
        url: str | None = None,
        auto_fix: bool | AutoFixMode | None = None,
        ...
        Args:
            ...
            auto_fix: Override the constructor's auto_fix setting for
                this run only. Pass ``AutoFixMode.ALWAYS`` to force
                regeneration (discards any cached script).
    ...
    async def async_regenerate(
        ...
        Equivalent to ``async_run(auto_fix=AutoFixMode.ALWAYS)``.
        """
        self._check_script_protection(force=force)
        return await self.async_run(url=url, auto_fix=AutoFixMode.ALWAYS)
    ...
    def regenerate(
        ...
        Equivalent to ``run(auto_fix=AutoFixMode.ALWAYS)``.
        """
        self._check_script_protection(force=force)
        return self.run(url=url, auto_fix=AutoFixMode.ALWAYS)
    ...
    async def _run_generate(
        ...
        agent = AgentLoop(
            ...
            max_script_attempts=self._max_retries + 1,
            ...
            disable_validator=not self._validator_agent,
            inputs=input_values,

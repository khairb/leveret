...
class ScraperResult:
    ...
    Attributes:
        ...
        script_generated: ``True`` if a new script was generated by the AI
            agent on this run (costs API credits). ``False`` if an
            existing cached script was reused (free).
        script_path: Absolute path to the script file, or ``None``
            if ``script=`` was not set on the Scraper.
        auto_regenerated: ``True`` when auto-regeneration triggered script
            regeneration. Implies ``script_generated=True`` (a fresh script
            was generated).
    """
    ...
    def __repr__(self) -> str:
        parts = [
            ...
            f"script_generated={self.script_generated}",
        ]
        if self.auto_regenerated:
            parts.append(", auto_regenerated=True")
        parts.append(")")
        return "".join(parts)
...
class Scraper:
    ...
    The first ``run()`` uses an AI model to generate a scraping script,
    validates it against your schema, and saves it to disk. Every
    subsequent ``run()`` loads the saved script — no AI, no API cost.

    Args:
        url: Target URL (must start with ``https://`` or ``http://``).
        task: Plain-English description of what to extract, e.g.
            ``"Extract all product listings with title and price"``.

        schema: Expected data shape. Accepts plain types, dicts,
            ``Field``, or ``Items``::

                schema={"title": str, "price": float}
                schema=Items({"title": str}, min_items=10)

        schema_tolerance: How strictly to validate list items against the
            schema. ``STRICT`` requires 100%, ``BALANCED`` (default)
            requires 80%, ``LENIENT`` requires 50%.

        script: File path to save/load the generated script. Without
            this, every ``run()`` generates from scratch and costs API
            credits. Example: ``script="scrapers/hn.py"``.
        protect_manual_edits: When ``True``, prevents ``auto_regenerate``
            and ``regenerate()`` from overwriting a script you edited by
            hand. Use ``regenerate(force=True)`` to override.

        browser: Shared :class:`Browser` instance for running multiple
            scrapers in the same browser. Mutually exclusive with
            ``headless`` and ``launch_options`` (set those on the
            Browser instead).
        headless: Run the browser without a visible window. Defaults
            to ``True``. When ``demo=True``, this is implicitly
            ``False`` (the browser must be visible for the overlay).
        launch_options: Patchright/Playwright browser launch options
            — locale, proxy, viewport, geolocation, etc. See
            :class:`LaunchOptions` for available fields.
        demo: Show the agent working in a visual side-by-side view
            with a live overlay panel displaying the agent's thinking,
            plan, and progress. Implies ``headless=False``.

        model: LLM model for script generation. Defaults to
            ``"claude-haiku-4-5"``. Use ``"provider:model"`` format
            for non-Anthropic models (e.g. ``"openai:gpt-4o"``).
        api_key: API key for the model provider. Falls back to the
            standard environment variable (e.g. ``ANTHROPIC_API_KEY``).

        run_timeout: Maximum seconds for script execution. Defaults to
            600 (10 minutes). Does not limit AI generation time.
        generation_attempts: Maximum number of AI generation attempts.
            Defaults to 6.
        auto_regenerate: Automatically diagnose and regenerate broken
            cached scripts. ``False`` (default) disables auto-regeneration.
            ``True`` enables balanced mode. Pass a :class:`RegenerateMode`
            for fine-grained control.
        stop_on_captcha: When ``True`` (default), the agent stops
            gracefully if it detects a CAPTCHA instead of looping.

        sandboxed: Run AI-generated code in a restricted sandbox that
            blocks filesystem, network, and system access. Enabled by
            default. Disable only if you trust the target site.
        ai_review: Use an additional AI agent to review the
            generated script's quality before accepting it. Disabled
            by default (the schema validation is always performed
    ...
    Example::
        ...
        scraper = Scraper(
            ...
            schema=Items({"title": str, "url": str}, min_items=10),
    ...
    def __init__(
        ...
        *,
        ...
        generation_attempts: int = _SENTINEL,
        generation_timeout: int | None = None,
        ai_review: bool = _SENTINEL,
        # ── Execution ──
        run_timeout: int = _SENTINEL,
        stop_on_captcha: bool = True,
        # ── Safety ──
        sandboxed: bool = _SENTINEL,
        # ── Deprecated names (backward compat) ──
        tolerance: Tolerance | str = _SENTINEL,
        protect_script: bool = _SENTINEL,
        auto_fix: bool | RegenerateMode = _SENTINEL,
        max_retries: int = _SENTINEL,
        validator_agent: bool = _SENTINEL,
        timeout: int = _SENTINEL,
        ...
        # -- schema_tolerance --
        resolved_tolerance = self._resolve_tolerance_value(schema_tolerance)
        ...
        if not isinstance(generation_attempts, int) or generation_attempts < 1:
            raise Error(
                f"generation_attempts must be a positive integer (got {generation_attempts!r})"
            )
        ...
        # -- auto_regenerate --
        auto_regenerate_mode = None
        if auto_regenerate is not False:
            auto_regenerate_mode = self._resolve_auto_regenerate_value(auto_regenerate)

            # §3/§11: auto_regenerate without script= has no effect
            # (except "always" which forces regeneration regardless)
            if script_path is None and auto_regenerate_mode != RegenerateMode.ALWAYS:
                logger.warning(
                    "auto_regenerate has no effect without script= — "
        ...
        # Demo mode implies headless=False (visible browser with overlay).
        ...
        self._schema_tolerance = resolved_tolerance
        ...
        self._auto_regenerate_mode = auto_regenerate_mode
        self._sandboxed = sandboxed
        self._protect_manual_edits = protect_manual_edits
    ...
    # Mapping from user-facing auto_regenerate values to RegenerateMode enums.
    _VALID_AUTO_REGENERATE = {
        True: RegenerateMode.BALANCED,
        "balanced": RegenerateMode.BALANCED,
        "cautious": RegenerateMode.CAUTIOUS,
        "eager": RegenerateMode.EAGER,
        "always": RegenerateMode.ALWAYS,
        # Backward compat for old string values
        "conservative": RegenerateMode.CAUTIOUS,
        "aggressive": RegenerateMode.EAGER,
    }
    ...
    @staticmethod
    def _resolve_auto_regenerate_value(value: bool | str | RegenerateMode) -> RegenerateMode:
        """Convert a user-facing auto_regenerate value to a RegenerateMode enum.

        Raises ConfigError for invalid values.
        """
        # Accept RegenerateMode enum values directly
        if isinstance(value, RegenerateMode):
            return value
        mode = Scraper._VALID_AUTO_REGENERATE.get(value)
        if mode is None:
            raise ConfigError(
                f"auto_regenerate must be False, True, or a RegenerateMode value "
                f"(got {value!r}). Examples:\n\n"
                f"  auto_regenerate=True                     # balanced mode\n"
                f"  auto_regenerate=RegenerateMode.CAUTIOUS   # cautious mode\n"
                f"  auto_regenerate=RegenerateMode.EAGER      # eager mode\n"
                f"  auto_regenerate=RegenerateMode.ALWAYS     # regenerate every run"
            )
        return mode
    ...
    _VALID_TOLERANCE: dict[str, Tolerance] = {
        ...
        "lenient": Tolerance.LENIENT,
        "tolerant": Tolerance.TOLERANT,  # backward compat
    }
    ...
    async def async_run(
        ...
        *,
        url: str | None = None,
        auto_regenerate: bool | RegenerateMode | None = _SENTINEL,
        inputs: dict[str, Any] | None = None,
        # Deprecated
        auto_fix: bool | RegenerateMode | None = _SENTINEL,
    ) -> ScraperResult:
        ...
        Args:
            ...
            auto_regenerate: Override the constructor's auto_regenerate
                setting for this run only. Pass ``RegenerateMode.ALWAYS``
                to force regeneration (discards any cached script).
        ...
        # Resolve effective auto_regenerate mode for this call
        if auto_regenerate is not None:
            if auto_regenerate is False:
                effective_mode = None
            else:
                effective_mode = self._resolve_auto_regenerate_value(auto_regenerate)
        else:
        ...
        if force_regenerate and self._script_path and self._script_path.exists():
            ...
            # regenerate(); this covers run(auto_regenerate="always") directly)
            if _is_script_user_edited(self._script_path):
                if self._protect_manual_edits:
                    raise ConfigError(
                        f"Script '{self._script_path}' has been manually "
                        f"edited and protect_manual_edits=True.\n\n"
                        f"  To overwrite:  scraper.regenerate(force=True)\n"
                        f"  To keep edits: set protect_manual_edits=False"
                    )
                logger.warning(
    ...
    def run(
        ...
        *,
        ...
        """
        _kw: dict[str, Any] = {"url": url, "inputs": inputs}
        if auto_regenerate is not _SENTINEL:
            _kw["auto_regenerate"] = auto_regenerate
        if auto_fix is not _SENTINEL:
            _kw["auto_fix"] = auto_fix
    ...
    async def async_regenerate(
        ...
        *,
        ...
        Args:
            url: Override the constructor URL for this execution only.
            force: Override ``protect_manual_edits`` for this call. Required
                when ``protect_manual_edits=True`` and the script has been
                edited by the user.
            inputs: Dynamic input values to pass through to generation.

        Equivalent to ``async_run(auto_regenerate=RegenerateMode.ALWAYS)``.
        """
    ...
    def regenerate(
        ...
        *,
        ...
        Equivalent to ``run(auto_regenerate=RegenerateMode.ALWAYS)``.
        """
        self._check_script_protection(force=force)
        return self.run(url=url, auto_regenerate=RegenerateMode.ALWAYS, inputs=inputs)
    ...
    def _check_script_protection(self, *, force: bool = False) -> None:
        ...
        Raises ConfigError if ``protect_manual_edits=True`` and the script
        exists and was edited by the user, unless ``force=True``.
        Always warns (regardless of protect_manual_edits) if user edits are
        detected.
        """
        ...
        if self._protect_manual_edits and not force:
            raise ConfigError(
                f"Script '{self._script_path}' has been manually edited "
                f"and protect_manual_edits=True.\n\n"
                f"  To overwrite anyway:  scraper.regenerate(force=True)\n"
                f"  To keep your edits:   set protect_manual_edits=False or "
    ...
    async def _run_cached(
        self,
        effective_url: str,
        auto_regenerate_mode: RegenerateMode | None = None,
        ...
        if auto_regenerate_mode is not None:
            return await self._run_cached_with_autofix(effective_url, auto_regenerate_mode)
        ...
        if self._context_managed or self._shared_browser is not None:
            ...
            if returncode == -1 and "timed out" in stderr:
                raise ScriptTimeoutError(
                    f"Script exceeded the {self._run_timeout}s timeout.\n\n"
                    f"  Increase the timeout:\n"
                    f"    Scraper(..., run_timeout={self._run_timeout * 2})"
                )
    ...
    async def _run_cached_with_autofix(
        ...
        assert auto_regenerate_mode is not None
        ...
        # Respect protect_manual_edits: if user edited the script, don't
        # auto-overwrite — raise the original error instead.
        if (
            self._protect_manual_edits
            ...
            logger.info(
                ...
                "(manually edited + protect_manual_edits=True). Raising error."
            )
            self._raise_autofix_error(result)
        ...
        try:
            ...
            # Re-raise as ValidationError (not AutoRegenerateError)
        ...
        regen_result.auto_regenerated = True
        return regen_result
    ...
    async def _run_in_process(
        ...
        if self._shared_browser is not None:
            page = await self._shared_browser._new_page()
        else:
        ...
        try:
            ...
            try:
                ...
                data = await asyncio.wait_for(call, timeout=self._run_timeout)
        ...
        try:
            ...
            try:
                ...
                return (
                    "",
                    None,
                    f"Function timed out after {self._run_timeout} seconds",
                    -1,
                )
    ...
    async def _run_generate(
        ...
        agent = AgentLoop(
            ...
            headless=self._headless,
            script_timeout=self._run_timeout,
            max_script_attempts=self._generation_attempts,
            ...
            disable_validator=not self._ai_review,
            inputs=input_values,
            input_defs=input_defs,
        )
